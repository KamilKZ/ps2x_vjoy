...
/****************************************************************************************/
boolean PS2X::NewAnalogStickState() {
  return ((last_analog_stick ^ analog_stick) > 0);
}

/****************************************************************************************/
boolean PS2X::NewAnalogDPadState() {
  return ((last_analog_dpad ^ analog_dpad) > 0);
}

/****************************************************************************************/
boolean PS2X::NewAnalogFaceButtonState() {
  return ((last_analog_face_button ^ analog_face_button) > 0);
}

/****************************************************************************************/
boolean PS2X::NewAnalogBackButtonState() {
  return ((last_analog_back_button ^ analog_back_button) > 0);
}

/****************************************************************************************/
unsigned long PS2X::AnalogStickByte() {
   return (analog_stick);
}

/****************************************************************************************/
unsigned long PS2X::AnalogDPadByte() {
   return (analog_dpad);
}

/****************************************************************************************/
unsigned long PS2X::AnalogFaceButtonByte() {
   return (analog_face_button);
}

/****************************************************************************************/
unsigned long PS2X::AnalogBackButtonByte() {
   return (analog_back_button);
}

void PS2X::getData(unsigned char* p_data) {
	 p_data = PS2data;
}
...
boolean PS2X::read_gamepad(boolean motor1, byte motor2) {
   ...
   last_analog_stick 		= analog_stick;
   last_analog_dpad 		= analog_dpad;
   last_analog_face_button 	= analog_face_button;
   last_analog_back_button 	= analog_back_button;

#if defined(__AVR__)
   buttons = *(uint16_t*)(PS2data+3);   //store as one value for multiple functions
   analog_stick = *(uint32_t*)(PS2data+5);   //store as one value for multiple functions
   analog_dpad = *(uint32_t*)(PS2data+9);   //store as one value for multiple functions
   analog_face_button = *(uint32_t*)(PS2data+13);   //store as one value for multiple functions
   analog_back_button = *(uint32_t*)(PS2data+17);   //store as one value for multiple functions
#else
   buttons =  (uint16_t)(PS2data[4] << 8) + PS2data[3];   //store as one value for multiple functions
   analog_stick =  (uint32_t)(PS2data[9] << 24) + (uint32_t)(PSdata[8] << 16) + (uint32_t)(PSdata[7] << 8) + PS2data[6];
   analog_dpad =  (uint32_t)(PS2data[13] << 24) + (uint32_t)(PSdata[12] << 16) + (uint32_t)(PSdata[11] << 8) + PS2data[10];
   analog_face_button =  (uint32_t)(PS2data[17] << 24) + (uint32_t)(PSdata[16] << 16) + (uint32_t)(PSdata[15] << 8) + PS2data[14];
   analog_back_button =  (uint32_t)(PS2data[21] << 24) + (uint32_t)(PSdata[20] << 16) + (uint32_t)(PSdata[19] << 8) + PS2data[18];
#endif
