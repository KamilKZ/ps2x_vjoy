...
class PS2X {
  public:
    ...
    byte Analog(byte);
	//4 bytes (32 bits) each
	unsigned long AnalogStickByte(); 		// 1 byte per axis, 2 axes per stick, 2 sticks
	unsigned long AnalogDPadByte(); 		// 1 byte per button, 4 buttons
	unsigned long AnalogFaceButtonByte(); 	// 1 byte per button, 4 buttons
	unsigned long AnalogBackButtonByte(); 	// 1 byte per button, 4 buttons
	//gives 16 bytes (128 bits) total
	boolean NewAnalogStickState();
	boolean NewAnalogDPadState();
	boolean NewAnalogFaceButtonState();
	boolean NewAnalogBackButtonState();
    void reconfig_gamepad();
	void getData(unsigned char* p_data);
	void writeData();

	// best case scenario, with only analog sticks and the rest of digital buttons (DualShock 1)
	// 16 bits for buttons + 16 bits per stick (32 bits)
	// gives 6 bytes (48 bits)
	// at a rate of 60Hz that's 2880bits/s or 360bytes/s

	// worst case scenario, with all analog inputs and mandatory digital buttons (DualShock 2)
	// 16 bits for buttons + 128 bits for analogs
	// gives 18 bytes (144 bits)
	// @ 60Hz: 8640bits/s or 1080bytes/s
  ...
  private:
	...
	unsigned long last_analog_stick;
	unsigned long analog_stick;
	unsigned long last_analog_dpad;
	unsigned long analog_dpad;
	unsigned long last_analog_face_button;
	unsigned long analog_face_button;
	unsigned long last_analog_back_button;
	unsigned long analog_back_button;
